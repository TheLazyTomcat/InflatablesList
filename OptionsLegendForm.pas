...
const
  STAT_OPT_CMDS: array[0..Pred(Length(IL_STATIC_OPTIONS_TAGS))] of String = (
...
For i := Low(IL_STATIC_OPTIONS_TAGS) to High(IL_STATIC_OPTIONS_TAGS) do
  begin
    // tag
    TempLabel := AddLabel(grbStaticOptions,LABELS_SPACE,TempInt,[fsBold],IL_STATIC_OPTIONS_TAGS[i]);
    // command
